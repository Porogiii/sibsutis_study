...
void CodePrint(){
	...
	for (int i = 0; i < alphabet_num; i++) {
        // Используем условие проверки, что строка не пуста
        if (B[i].l > 0) {
            // Заменяем символ B[i].a на "-" при i = 56 или 57
            ...
            // Выводим "-" вместо пустого символа во второй колонке
            printf("| %2d |    -   |  %2.6f   |", i, B[i].q);
            for (int j = 0; j < 12; j++)  // Уменьшаем на 1, чтобы учесть "-"
                printf(" ");
            printf("|  %4d  |\n", B[i].l);
            lgm += B[i].p * B[i].l; // уточнено место для lgm в случае пустой строки
        }
    }
